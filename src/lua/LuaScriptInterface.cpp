...
void LuaScriptInterface::Init()
{
	...
	{
		luacon_ci->Log(CommandInterface::LogWarning, "No autorun.lua found");
	}
...
void LuaScriptInterface::initSimulationAPI()
{
	...
	struct luaL_Reg simulationAPIMethods [] = {
		...
		{"loadSaveFile", simulation_loadSaveFile},
...
int LuaScriptInterface::simulation_loadSaveFile(lua_State * l)
{
	const char* filename = luaL_optstring(l, 1, "");
	//luacon_controller->LoadSaveFile(filename);
	...
	if(!gameSaveData.size())
		...
		else
		{
			...
			luacon_controller->LoadSaveFile(newFile);
			delete newFile;
		}
