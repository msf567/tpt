...
void Element::Element_CLIP()
{
    ...
    Description = "Plays a clip when sparked";
        ...
        for (searchY = -1; searchY <= 1; searchY++)
            if (BOUNDS_CHECK && (searchX || searchY))
            {
                ...
                if (TYP(targetParticle) == PT_SPRK)
                {
                    ...
                    {
                        parts[i].life = 6;
                        CLIP::get_clip_player()->playSoundFromMemory(NULL, SDL_MIX_MAXVOLUME / 20); // temp
