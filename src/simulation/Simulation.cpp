...
int Simulation::LoadNextSave()
{
	if (saveIndex >= filePaths.size())
		saveIndex = filePaths.size() - 1;

	std::vector<unsigned char> gameSaveData = Client::Ref().ReadFile(filePaths[saveIndex]);
	...
	else
	{
		...
		newSave->paused = false;
		clear_area(0, 0, XRES, YRES);
		Load(newSave, true);
	}
		...
		if (parts[i].type)
		{
			...
			if (!legacy_enable)
			{
...
#endif
				{
					...
					if (((elements[t].Properties & TYPE_LIQUID) && IsValidElement(elements[t].HighTemperatureTransition) && (elements[elements[t].HighTemperatureTransition].Properties & TYPE_GAS)) || t == PT_LNTG || t == PT_SLTW)
						ctemph -= 2.0f * pv[y / CELL][x / CELL];
					else if (((elements[t].Properties & TYPE_GAS) && IsValidElement(elements[t].LowTemperatureTransition) && (elements[elements[t].LowTemperatureTransition].Properties & TYPE_LIQUID)) || t == PT_WTRV)
